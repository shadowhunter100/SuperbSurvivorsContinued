...
function AIManager(TaskMangerIn)
	...
	if ((getSpecificPlayer(0) == nil) or (not getSpecificPlayer(0):isAsleep())) and (ASuperSurvivor:getAIMode() ~= "Stand Ground") then
		...
		if (ASuperSurvivor:getCurrentTask() == "None") and (IsInBase) and (not IsInAction) and (ZombRand(4) == 0) then
			if (not SurvivorCanFindWork) and (ASuperSurvivor:getGroupRole() == "Doctor") then
				...
				if (randresult == 1) then
					ASuperSurvivor:Speak(Get_SS_UIActionText("IGoRelax"))
					TaskMangerIn:AddToTop(WanderInBaseTask:new(ASuperSurvivor))
				else
			...
			elseif (not SurvivorCanFindWork) and (ASuperSurvivor:getGroupRole() == "Farmer") then
				if (SurvivorCanFindWork) and (RainManager.isRaining() == false) then
					...
					if (randresult == 1) then
						...
						if (area) then
							ASuperSurvivor:Speak(Get_SS_UIActionText("IGoFarm"))
				...
				if (ASuperSurvivor:Get():getBodyDamage():getWetness() < 0.2) then
					if (SafeToGoOutAndWork) then
						...
						elseif (job == "Gather Wood") then
							ASuperSurvivor:Speak(Get_SS_UIActionText("IGoGetWood"))
						...
						elseif (job == "Pile Corpses") then
							ASuperSurvivor:Speak(Get_SS_UIActionText("IGoPileCorpse"))
						...
						elseif (job == "Forage") then
							...
							if (forageSquare ~= nil) then
								ASuperSurvivor:Speak(Get_SS_UIActionText("IGoForage"))
						...
						elseif (job == "Chop Wood") then
							if (chopWoodSquare) then
								ASuperSurvivor:Speak(Get_SS_UIActionText("IGoChopWood"))
						...
						elseif (job == "Guard") then
							if (guardArea) then
								ASuperSurvivor:Speak(Get_SS_UIActionText("IGoGuard"))
		...
		if (ASuperSurvivor:getCurrentTask() == "None") and (IsInBase == false) and (not IsInAction) and (HisGroup ~= nil) then
			...
			if (baseSq ~= nil) then
				ASuperSurvivor:Speak(Get_SS_UIActionText("IGoBackBase"))
	...
	then -- solo random survivor AI flow	
		...
		then -- leave group and look for food if starving
			...
			ASuperSurvivor:getTaskManager():clear()
			ASuperSurvivor:Speak(Get_SS_UIActionText("LeaveBCHungry"))

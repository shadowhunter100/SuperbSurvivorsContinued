...
function SuperSurvivorsOnTick()
	if (SuperSurvivorSelectAnArea) then
		...
		if (Mouse.isLeftPressed()) then
			SuperSurvivorSelectAreaHOLD = false; -- WIP - WHO DID THIS? - I did a folder scan, this var doesn't do anything?
...
function SuperSurvivorsInit()
	...
	-- No Reason to have achievements ... that is beyond the scope of having followers and npcs.
...
-- WIP - WHAT IS THIS FUNCTION SUPPOSED TO DO?
		...
		if (SS) and not player:isLocalPlayer() then
			...
			if (weapon:isRoundChambered()) then
				...
				getSoundManager():PlayWorldSound(
					weapon:getSwingSound(), player:getCurrentSquare(), 0.5, range, 1.0, false
				);
	...
	if (getSpecificPlayer(0)) then
		...
		elseif (keyNum == 0) then
			...
			for q = 1, 2 do
				...
				if movePenalty < 0 then
					movePenalty = 0;
				end
				if attacker:HasTrait("Marksman") then
...
-- WIP - What IS THIS SUPPOSED TO DO AND WHY?
...
function SuperSurvivorsNewSurvivorManager()
	...
	for i = 1, 10 do
		...
		if (spawnSquare ~= nil) and (not hisGroup:IsInBounds(spawnSquare))
			and spawnSquare:isOutside()
			and (not spawnSquare:IsOnScreen())
			and (not spawnSquare:isSolid()) and (spawnSquare:isSolidFloor())
		then
			success = true;
			break;
		end
...
function SuperSurvivorDoRandomSpawns()
	...
	if (DebugOptions == true) then
		DebugSuperSurvivorDoRandomSpawns();
	else
...
-- WIP - WHO MADE THIS? WHAT IS THE SPAWN RATE AND HOW IS IT DEFINED?
...
function SuperSurvivorsRaiderManager()
	...
	-- WIP - Why does this need to be run?
	--this unrelated to raiders but need this to run every once in a while
	getSpecificPlayer(0):getModData().hitByCharacter = false;
	getSpecificPlayer(0):getModData().semiHostile = false;
	getSpecificPlayer(0):getModData().dealBreaker = nil
...
-- Cows: I guess this is the "compatibility check" with the last workshop item update in February 2023...
-- WIP - Commented out, seems unnecessary as PBKAC is not in the scope of this work.
...
function DebugSuperSurvivorDoRandomSpawns()
	local RealAlternativeSpawning = AlternativeSpawning - 1

	-- It will run through these only so many times depending on the options itself. Should reduce lag.
	for i = RealAlternativeSpawning, 1, -1 do

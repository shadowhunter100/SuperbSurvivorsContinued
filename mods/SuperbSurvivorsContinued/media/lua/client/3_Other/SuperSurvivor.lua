...
function SuperSurvivor:DoVision()
	...
	-- WIP - If the weapon is a gun then the range of the gun is the danger range... which can be bad if npc starts shooting and attracting more Zs
...
function SuperSurvivor:isInAction()
	...
	if ((self.player:getModData().bWalking == true) and (self.TicksSinceSquareChanged <= 10)) then
		--print(self:getName().." returing true;1");
		return true;
	end
	...
	for k = 1, #queue.queue do
		...
		if v then
			--print(self:getName().." returing true;2");
			return true;
		end
	end
...
function SuperSurvivor:NPCcalcRunSpeedModByBag(bag)
	;
	local speedModifier = bag:getScriptItem().runSpeedModifier - 1.0;
	local bagWeight = 1.0 + bag:getContentsWeight();
	local bagCapacity = bag:getEffectiveCapacity(self.player);

	return speedModifier * (bagWeight / bagCapacity / 2.0);
end
...
-- WIP - What is the function about?...

...
function SuperSurvivorGroup:getBestGroupAreaContainerForItem(item) -- returns most appropriate container(or square) for an item based on set base areas
	...
	local overrideTool = { "Axe", "Hammer", "Nails", "NailsBox", "BallPeenHammer", "Sledgehammer", "Sledgehammer2",
		"HammerStone", "GardenSaw", "Saw", "ScrewsBox", "Screws", "Screwdriver", "HandAxe", "HandShovel", "HandScyth" };
...
function SuperSurvivorGroup:IsInBounds(thisplayer)
	if (self.Bounds[4]) then
		if (thisplayer:getZ() == self.Bounds[5])
			and (thisplayer:getX() > self.Bounds[1])
			and (thisplayer:getX() <= self.Bounds[2])
			and (thisplayer:getY() > self.Bounds[3])
			and (thisplayer:getY() <= self.Bounds[4])
		then
			return true;
		else
			return false;
	...
	for i = 1, #self.Members do
		...
		if (workingID ~= nil) then
			...
			if (SSM:Get(workingID):isInAction() == false)
				and (distance ~= 0) and (distance < closestSoFar)
				and ((SSM:Get(workingID):getGroupRole() == ofThisRole) or (ofThisRole == "Any") or (ofThisRole == nil))
			then
				closestID = workingID;
...
function SuperSurvivorGroup:getIdleMember(ofThisRole, closest)
	...
	for i = 1, #self.Members do
		...
		if (workingID ~= nil) and (SSM:Get(workingID):isInAction() == false)
			and ((SSM:Get(workingID):getGroupRole() == ofThisRole)
				or (ofThisRole == "Any")
				or (ofThisRole == nil))
		then
...
function SuperSurvivorGroup:stealingDetected(thief)
	for i = 1, #self.Members do
		...
		if (workingID ~= nil) and (thief ~= nil) and (thief:getModData().ID ~= nil)
			and (workingSS ~= nil) and (workingSS:getGroupID() == self.ID) then
			if (self:getWarnPlayer(thief:getModData().ID))
				and SSM:Get(workingID):Get():CanSee(thief)
			then

...
function InventoryRow:createChildren()
    ...
    local cat_transfer = ISButton:new(cat_item.x + cat_item.width, 0, 102, 25,
        self.direction == "v" and "unequip" or "equip", nil, function() self:on_click_transfer(self.direction) end)
...
function create_panel_loadout(member_index)
    ...
    panel_loadout = PanelLoadout:new(window_super_survivors.x + window_super_survivors.width + 8,
        window_super_survivors.y, 426, 25 * 15 + (3 * 2), member_index)

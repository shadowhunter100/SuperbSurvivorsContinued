...
table.insert(inventoryPanels, 1, PanelMemberInventory); -- Cows: NPC inventory is inserted first.

function PanelMemberInventory:dupdate()
    local isLocalFunctionLoggingEnabled = false;
	CreateLogLine("SuperSurvivorInventory", isLocalFunctionLoggingEnabled, "function: PanelMemberInventory:dupdate() called");
...
function PanelOwnInventory:dupdate()
    local isLocalFunctionLoggingEnabled = false;
	CreateLogLine("SuperSurvivorInventory", isLocalFunctionLoggingEnabled, "function: PanelOwnInventory:dupdate() called");
    -- Cows: There should be a better way.. because if an inventory has over > 100 items, that means > 100 new items are being re-rendered every time...
    self:clearChildren();
    local dy = 0;
    local switch = 0;

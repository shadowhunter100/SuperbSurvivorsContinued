...
function SurenderTask:isComplete()
	...
	local facingResult = self.enemy.player:getDotWithForwardDirection(self.parent.player:getX(),
		self.parent.player:getY())
	...
	local result = (not (self.enemy.player:isAiming()
			...
			and (self.parent:isEnemy(self.enemy.player)
				or ((not self.parent:isInGroup(self.enemy.player))
...
function SurenderTask:update()
	...
	if ((self.enemy.player:isAiming() and self.enemy:usingGun()
			and (self.parent:isEnemy(self.enemy.player)
				or (self.enemy.player:isLocalPlayer()
					and IsoPlayer.getCoopPVP()))
			and GetDistanceBetween(self.parent.player, self.enemy.player) < 6))
	then

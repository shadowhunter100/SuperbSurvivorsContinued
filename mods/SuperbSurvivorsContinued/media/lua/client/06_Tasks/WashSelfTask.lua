...
function WashSelfTask:new(superSurvivor)
	...
	superSurvivor:RoleplaySpeak(Get_SS_UIActionText("LookForItem_Before") .. "Wash Water" .. Get_SS_UIActionText("LookForItem_After"))
	return o
end
...
function WashSelfTask:update()
	...
	if (self.TargetItem == nil) then
		...
		if (self.WasSuccessful == false) then
			...
			"Wash Water" .. Get_SS_UIActionText("NoFindItem_After"))
		end
		return false
	end
				...
				for i = 1, items:size() - 1 do
					...
					if (bloodAmount > 0 or dirtAmount > 0) then
						if waterRemaining > ISWashClothing.GetRequiredWater(item) then
							...
							ISTimedActionQueue.add(ISWashClothing:new(self.parent.player, self.TargetItem, soapList, item,
								bloodAmount, dirtAmount, noSoap))
						end
					end
				end
			end

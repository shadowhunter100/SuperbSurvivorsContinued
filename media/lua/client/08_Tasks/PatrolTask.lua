...
function PatrolTask:new(superSurvivor, square1, square2)
	...
	elseif (superSurvivor.player:getModData().PX ~= nil) then
		o.Square1 = getCell():getGridSquare(superSurvivor.player:getModData().PX, superSurvivor.player:getModData().PY,
			superSurvivor.player:getModData().PZ)
	else
		o.Square1 = superSurvivor.player:getCurrentSquare()
	end
	...
	elseif (superSurvivor.player:getModData().P2X ~= nil) then
		o.Square2 = getCell():getGridSquare(superSurvivor.player:getModData().P2X, superSurvivor.player:getModData().P2Y,
			superSurvivor.player:getModData().P2Z)
	else
		return nil
	end
